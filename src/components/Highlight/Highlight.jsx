...
function Highlight({ type, image, children }) {
  ...
  return (
<div
  className={`flex max-sm:flex-col sm:flex-row ${
    type == 1 ? "sm:flex-row" : "sm:flex-row-reverse"
  } gap-40 px-20 h-highlight overflow-hidden my-20`}
>
  <div className="flex-col gap-10 w-full max-sm:w-full sm:w-1/2">
    {type == 1 ? (
      <h2 className="ml-24 text-header font-thin">
        Vision for Holistic Development
      </h2>
    ) : null}
    {children}
  </div>
  <div className="w-full max-sm:w-full sm:w-1/2 h-24">
